...
int main(void)
{
    ...
    char testi[crypto_core_ristretto255_HASHBYTES];
    unsigned char testipx[crypto_core_ristretto255_BYTES];
    randombytes_buf(testi, sizeof testi);
    unsigned char testix = (unsigned char)*testi;

    crypto_core_ristretto255_from_hash(testipx, &testix);
...
// Initialize the unsigned char px here
...
// Free the memory assigned to px.

...
void testInvalidSecondArgPointScalarmultristretto_init() {
    ...
    my_bool result = scalarmultristretto_init(&initid, &args, message);
    assert(result == 1 && "Result is not 1, _init passed when it should have failed.");
    assert(strcmp(message, "Second input argument is not a valid ristretto point") == 0 && "Error message is incorrect");
    assert(initid.ptr == 0 && "Memory was allocated when it shouldn't");
...
void testScalarmultristretto() {
    ...
    char error[1];
    char is_null[1];
    char *returnedPtr = scalarmultristretto(&initid, &args, result, length, is_null, error);
    assert(returnedPtr == initid.ptr && "Returned pointer does not originate from the UDF_INIT struct");
    assert(crypto_core_ristretto255_is_valid_point(ristrettoPoint) == 1 && "Output of the scalarmultristretto() is not a valid ristretto point");
    char expecteRistrettoPoint[crypto_core_ristretto255_BYTES];
    crypto_scalarmult_ristretto255(expecteRistrettoPoint, scalar, point);
    assert(memcmp(expecteRistrettoPoint, ristrettoPoint, crypto_core_ristretto255_BYTES) == 0 && "Output of the scalarmultristretto() is not as expected");

...
void testScalarmultristretto_init()
{
    ...
    const my_bool result = scalarmult_ristretto255_init( &initid, &args, message );
    assert( result == false &&
...
void testScalarmultristretto()
{
    ...
    char error[1];
    char is_null[1];
    const char* returnedPtr = scalarmult_ristretto255( &initid, &args, result,
                              length,
                              is_null, error );
...
int main()
{
    ...
    testInvalidSecondArgSizeScalarmultristretto();
    return 0;
}
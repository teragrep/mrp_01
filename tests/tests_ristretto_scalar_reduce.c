...
void testRistrettoScalarReduce_init()
{
    ...
    const my_bool result = ristretto_scalar_reduce_init( &initid, &args, message );
    assert( result == false &&
...
void testRistrettoScalarReduce()
{
    ...
    const char* returnedPtr = ristretto_scalar_reduce( &initid, &args, result,
                              length,
                              is_null, error );
...
void testInvalidFirstArgSizeRistrettoScalarReduce()
{
    unsigned char nonReducedScalar[16];
    ...
    printf( "testInvalidFirstArgSizeRistrettoScalarReduce() passed assertions!\n" );
}
...
int main()
{
    ...
    testInvalidFirstArgSizeRistrettoScalarReduce();
    return 0;
}
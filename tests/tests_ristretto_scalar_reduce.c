...
void testRistrettoScalarReduce_init() {
    char *nonReducedScalar = malloc(crypto_core_ristretto255_NONREDUCEDSCALARBYTES);
...
void testInvalidFirstArgSizeRistrettoScalarReduce_init() {
    ...
    my_bool result = ristrettoscalarreduce_init(&initid, &args, message);
    assert(result == 1 && "Result is not 1, _init passed when it should have failed.");
    assert(strcmp(message, "First input is not a scalar in 64 byte binary string format") == 0 && "Error message is incorrect");
    assert(initid.ptr == 0 && "Memory was allocated when it shouldn't");
...
void testRistrettoScalarReduce() {
    ...
    assert(scalar != 0);
    UDF_INIT initid = {.maybe_null=0, .decimals = 3, .max_length = crypto_core_ristretto255_SCALARBYTES, .ptr = scalar, .const_item = 0};
    char * returnedPtr = ristrettoscalarreduce(&initid, &args, result, length, is_null, error);
    ...
    assert(memcmp(expectedScalar, scalar, crypto_core_ristretto255_SCALARBYTES) == 0 && "Output of the ristrettoscalarreduce() is not as expected");

...
void testRistrettoScalarNegate() {
    ...
    assert(scalar != 0);
    UDF_INIT initid = {.maybe_null=0, .decimals = 3, .max_length = crypto_core_ristretto255_SCALARBYTES, .ptr = scalar, .const_item = 0};
    char *returnedPtr = ristrettoscalarnegate(&initid, &args, result, length, is_null, error);
    ...
    crypto_core_ristretto255_scalar_negate(expectedScalar, inputScalar);
    assert(memcmp(expectedScalar, scalar, crypto_core_ristretto255_SCALARBYTES) == 0 && "Output of the ristrettoscalarnegate() is not as expected");

...
void testRistrettoScalarComplement_init() {
    ...
    my_bool result = ristrettoscalarcomplement_init(&initid, &args, message);
...
void testRistrettoScalarComplement() {
    ...
    char *returnedPtr = ristrettoscalarcomplement(&initid, &args, result, length, is_null, error);
    ...
    crypto_core_ristretto255_scalar_complement(expectedScalar, inputScalar);
    assert(memcmp(expectedScalar, scalar, crypto_core_ristretto255_SCALARBYTES) == 0 && "Output of the ristrettoscalarcomplement() is not as expected");

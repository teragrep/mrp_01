...
void testRistrettoScalarMul() {
    ...
    char *returnedPtr = ristrettoscalarmul(&initid, &args, result, length, is_null, error);
    ...
    assert(memcmp(expectedScalar, scalar, crypto_core_ristretto255_SCALARBYTES) == 0 && "Output of the ristrettoscalarmul() is not as expected");
